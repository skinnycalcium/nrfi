...
Picks the side with positive edge and flags BET / NO BET / NO LINE / PITCHER TBD.
"""
...
def build_game(raw, season):
    ...
    # If either pitcher is TBD, return a stub Game so the matchup STILL appears in the email.
    # We won't compute model probabilities or a pick, but the user sees the game on the slate.
    if not (away_pp and home_pp):
        a_name = (away_pp or {}).get("fullName", "TBD")
        h_name = (home_pp or {}).get("fullName", "TBD")
        log(f"  ~ {a_name:24s} @ {h_name:24s} ({venue}) [TBD pitcher - keeping on slate]")
        ...
        stub = Game(
            ...
            away_pitcher=Pitcher(id=0, name=a_name),
            home_pitcher=Pitcher(id=0, name=h_name),
            away_lineup=Lineup(), home_lineup=Lineup(),
            dome=stadium["dome"],
        )
        stub.verdict = "PITCHER TBD"
        return stub
...
def score(g, odds):
    # Skip scoring for TBD-pitcher stub games - they keep their "PITCHER TBD" verdict.
...
def render_html(games, target_date):
    ...
    for g in games:
        vc = ("v-bet-nrfi" if g.verdict == "BET NRFI" else
              ...
              "v-nobet" if g.verdict == "NO BET" else
              "v-tbd" if g.verdict == "PITCHER TBD" else "v-nolne")

        if g.verdict == "PITCHER TBD":
            mblock = '<div class="market"><div class="m-row"><span>Status</span><span class="mono dim">Awaiting probable pitchers</span></div></div>'
        elif g.chosen_price is not None:
            ...
            <div class="market">
              ...
              <div class="m-row"><span>Kelly stake (1/4)</span><span class="mono">{stake_str}</span></div>
            </div>"""
        else:
        ...
        try:
            ...
            time_str = tlocal.strftime("%I:%M %p ET").lstrip("0")
        except Exception:
            time_str = ""

        if g.verdict == "PITCHER TBD":
            big_block = ('<div class="big"><div class="big-row">'
                         '<div><div class="big-label">P(NRFI)</div><div class="big-val dim">--</div><div class="big-sub">Pitchers TBD</div></div>'
                         '<div><div class="big-label">P(YRFI)</div><div class="big-val dim">--</div><div class="big-sub">Pitchers TBD</div></div>'
                         '</div></div>')
            grid_block = f"""
            <div class="grid2">
              <div class="col"><div class="col-name">{g.away_pitcher.name}</div><div class="row"><span>Stats</span><span class="mono dim">unavailable</span></div></div>
              <div class="col"><div class="col-name">{g.home_pitcher.name}</div><div class="row"><span>Stats</span><span class="mono dim">unavailable</span></div></div>
            </div>"""
        else:
            big_block = f"""
            <div class="big">
    ...
    gen = datetime.now(ET).strftime("%Y-%m-%d %I:%M %p ET")
...
.v-tbd{{color:var(--amber);border-color:var(--amber);background:rgba(245,158,11,0.08)}}
...
.footer{{margin-top:40px;padding-top:20px;border-top:1px solid var(--rule);font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--faint);text-align:center;letter-spacing:0.1em}}
  ...
  <header class="top">
    ...
    <div class="meta">GAMES SCANNED: {len(games)}<br/>GENERATED: {gen}<br/>MODEL: POISSON - v1.2</div>
  </header>
  <div class="summary">
    ...
    <div class="pill"><div class="l">Pitcher TBD</div><div class="v tbd">{tbd}</div></div>
    <div class="pill"><div class="l">Bankroll</div><div class="v">${BANKROLL:,.0f}</div></div>
  </div>
  <div class="cards">{''.join(cards) if cards else '<div class="empty">No games on this date.</div>'}</div>
...
def main():
    ...
    rank = {"BET NRFI": 0, "BET YRFI": 0, "NO BET": 1, "NO LINE": 2, "PITCHER TBD": 3}
    ...
    for g in games:
        if g.verdict == "PITCHER TBD":
            log(f"  {g.verdict:12s} {g.away_team[:14]:14s} @ {g.home_team[:14]:14s} (pitchers TBD)")
        elif g.chosen_price is not None:
            sign = "+" if g.chosen_price > 0 else ""
            log(f"  {g.verdict:12s} {g.away_team[:14]:14s} @ {g.home_team[:14]:14s} "
